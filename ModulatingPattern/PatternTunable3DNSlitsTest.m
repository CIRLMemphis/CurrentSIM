...
function [im, jm, Nm] = PatternTunable3DNSlitsTest(X, Y, Z, omegaXY, omegaZ, dXY, dZ, phiDeg, offsDeg, thetaDeg, phizDeg, Nslits)
...
for l = 1:Nthe
    ...
    im(:,:,:,l,:,2) = repmat(Visibility(zz*dZ, omegaZ(l), Nslits, phiz), [1 1 1 1 Nphi]);
    for k = 1:Nphi
        jm(:,:,:,l,k,2) = cos(xy + phi(k) + offs(l));
    end
end
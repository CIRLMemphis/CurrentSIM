function [reconOb, retFig, retVars] = HotOneShotLBFGSBNoDoubleTesting(...
        ...
        for l = 1:Nthe
            for k = 1:Nphi
                ...
                diffOb = g(:,:,:,l,k) - ForwardFct(curOb, h, imFct, jmFct, Nm, xx,     ...
                                                   yy, zz, dXY, dZ, curOmegaXY, omegaZ(k), theta(l), ...
                                                   curPhi, offs(l), phiz(k));
                ...
                for m = 1:Nm
                    dPhi = dPhi + IFT(FT(curOb.*jmTunable_dPhi(xx, yy, dXY, curOmegaXY, theta(l), curPhi, offs(l), m)).* ...
                                      FT(h.*imFct(zz, dZ, omegaZ(k), phiz(k), m)));
                end
...
lowerBd = [zeros(Y*X*Z,1); phi(1)-5; phi(2)-5; phi(3)-5; omegaXY(1) - omegaXY(1)*0.2; omegaXY(2) - omegaXY(2)*0.2; omegaXY(3) - omegaXY(3)*0.2];
upperBd = [  inf(Y*X*Z,1); phi(1)+5; phi(2)+5; phi(3)+5; omegaXY(1) + omegaXY(1)*0.2; omegaXY(2) + omegaXY(2)*0.2; omegaXY(3) + omegaXY(3)*0.2];

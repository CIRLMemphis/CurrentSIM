function curStepSize = PCStepSize(g0, ForwardFunction, CostFunction, xi, d, h, im, jm)
...
for k = 1:length(loc_alpha)
    if (isreal(loc_alpha(k)))
        cost_alpha(k) = CostFunction(g0, ForwardFunction, xi + loc_alpha(k)*d, h, im, jm);
    else
        cost_alpha(k) = realmax;
    end
end

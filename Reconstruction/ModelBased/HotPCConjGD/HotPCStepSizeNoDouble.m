function curStepSize = HotPCStepSizeNoDouble(ForwardModelFct, UpFct, g0, d, xi, h, imFunc, jmFunc, Nm, xx, yy, zz, dXY, dZ, omegaXY, omegaZ, thetaDeg, phiDeg, offDeg, phizDeg, penalizationParam)
if(nargin < 21)
    penalizationParam = -1;
end
[~, ~, ~, Nthe, Nphi] = size(g0);
...
for loc = 1:length(loc_alpha)
    if (isreal(loc_alpha(loc)))
        for l = 1:Nthe
            for k = 1:Nphi
                diff = g0(:,:,:,l,k) - ...
                       ForwardModelFct((xi + loc_alpha(loc)*d).^2, h, imFunc, jmFunc, Nm, xx,     ...

function curStepSize = HotPSFVzPCStepSizeMismatch(ForwardModelFct, UpFct, g0, d, xi, h, vz, jmFunc, Nm, xx, yy, zz, dXY, dZ, omegaXY, omegaZ, thetaDeg, phiDeg, offDeg, phizDeg, penalizationParam)
if(nargin < 21)
    penalizationParam = -1;
end
[~, ~, ~, Nthe, Nphi] = size(g0);
...
for l = 1:Nthe
    for k = 1:Nphi
        ...
        A3 = zeros(Y, X, Z);
        for m = 1:Nm
            jmTemp = jmFunc(xx, yy, zz, dXY, dZ, omegaXY(l), omegaZ(l), thetaDeg(l), phiDeg(k), offDeg(l), phizDeg, m, vz(:,:,:,m));
            FThim  = FT(h);

function [recon, retFig, retVars] = PCGradientDescent( ForwardFunction, CostFunction, Gradient, StepSize, g, h, im, jm, numIt, penalizationParam, initGuess, selectDir, picInterval)
fprintf('Call to %s\n',  mfilename);
...
tic;
for it = 1:numIt
    diff    = g2 - ForwardFunction(recon.^2, h, im, jm);
    curCrit = dot(diff(:), diff(:));
    curGrad = Gradient(diff, recon, h, im, jm);
    if (it == 1)
        memGrad  =  curGrad;
        desctDir = -curGrad;
    end
    ...
    alpha = StepSize(g2, ForwardFunction, CostFunction, recon, desctDir, h, im, jm);
    if(DEBUG_DISPLAY)
        fprintf('It : %d \n',it);

function [reconOb, retFig, retVars] = HotOneShotOffsetLBFGSBNoDouble(...
...
initGuess = initGuess/(Nphi*Nthe);
    ...
    function [crit, grad] = costGrad(x)
        curOb   = reshape(x(1:end-Nthe), Y,X,Z);
        gradOb  = zeros(X,Y,Z);
        gradOff = zeros(Nthe,1);
        crit    = 0;
        for l = 1:Nthe
            for k = 1:Nphi
                curOff = x(X*Y*Z + l);
                ...
                dPhi = zeros(X,Y,Z);
                for m = 1:Nm
                    dPhi = dPhi + IFT(FT(curOb.*jmFct_dPhi(xx, yy, dXY, omegaXY(l), theta(l), phi(k), curOff, m)).* ...
                                      FT(h.*imFct(zz, dZ, omegaZ(l), phiz, m)));
                end
                gradOff(l) = gradOff(l) + dot(diffOb(:), dPhi(:));
                crit       = crit + dot(diffOb(:), diffOb(:));
            end
        end
        grad = -2.*[reshape(real(gradOb), X*Y*Z, 1); real(gradOff)];
    end
...
for l = 1:Nthe
    lowerBd(end+1) = offs(l) - 180;
    upperBd(end+1) = offs(l) + 180;
    x0     (end+1) = offs(l);
end
...
reconOb = reshape(xRet(1:end-Nthe), Y, X, Z);
xRet(end-Nthe+1:end)
toc;
end

function [reconOb, retFig, retVars] = HotGradientDescentNoZ(...
...
for it = 1:numIt
    ...
    for l = 1:Nthe
        for k = 1:Nphi
            diff(:,:,:,l,k) = ForwardFct(  reconOb, h, imFct, jmFct, Nm, xx,     ...
                                yy, zz, dXY, dZ, omegaXY(l), omegaZ(l), theta(l), ...
                                phi(k), offs(l), phiz);
            diff(:,:,:,l,k) = UpFct(-diff(:,:,:,l,k), g(:,:,:,l,k));

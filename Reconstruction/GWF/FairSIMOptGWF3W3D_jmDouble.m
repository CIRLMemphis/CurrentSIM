function [Ob, retFig, retVars] = FairSIMOptGWF3W3D( g, h, im, uc, omegaXY, phiDeg, offsDeg, thetaDeg, wD, dXY, dZ, cutoff, A, sigma, returnVar, GFilter, inHn, weights, inHAtt)
...
cn = [1, 0.4*2, 0.4*2, 0.4, 0.4]; %cn = [1, 2/3, 2/3, 1/3, 1/3]; % cn = [1/3, 2/3, 1, 2/3, 1/3];
vn = ones(1, 1, Z, Nphi);
vn(1,1,:,2) = im(1,1,:,1,1,3);
vn(1,1,:,3) = im(1,1,:,1,1,3);

[Ob, figTits, retVars] = OptGWFCore(g,h,vn,qn,cn,uc,omegaXY,phiDeg,offsDeg,thetaDeg,wD,dXY,dZ,cutoff,A,sigma,weights,returnVar,GFilter,inHn,...

function [Ob, figTits, retVars] = OptGWFCore( g, h, vn, qn, cn, uc, omegaXY, phiDeg, offsDeg, thetaDeg, wD, dXY, dZ, cutoff, A, sigma, weights, returnVar, GFilter, inHn, inHAtt)
retVars = {};
figTits = {};
...
for l = 1:Nthe
    ...
    for k = 1 : Nphi
        ...
        if (qn(k) == 0)
            ...
            ObFT    = padarray(GM(:,:,:,l,k), [Y/2 X/2 floor(Z/2)]).*WFilter.*AD;
            WF      = WF + real(IFT(ObFT));
        end

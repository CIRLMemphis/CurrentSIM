...
load(CIRLDataPath + "\Results\3DBigStar\" + expNames(1) + "\" + expNames(1) + ".mat", 'g');
...
for k = 1:length(expNames)
    if (iterInd(k) == 0)
        load(CIRLDataPath + "\Results\3DBigStar\" + expNames(k) + "\" + expNames(k) + ".mat", 'reconOb');
        recVars{end+1} = reconOb./sum(reconOb(:))*sum(HROb(:));
    else
        load(CIRLDataPath + "\Results\3DBigStar\" + expNames(k) + "\" + expNames(k) + ".mat", 'retVars');

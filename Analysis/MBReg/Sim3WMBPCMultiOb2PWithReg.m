...
expName = '201910311726_Sim3WMBPCHotMultiOb2P256Iter200WithReg1e6';
...
Y2 = Y;
Z2 = Z;
load(CIRLDataPath + "/Simulation/3W/Sim3WMultiOb2P512.mat", 'ob');
HROb  = ob;
...
%norReconOb = retVars{4};
norReconOb = reconOb./sum(reconOb(:))*sum(norOb(:));
...
suptitle('Comparison of the true object and the MBPC with regularization results (not-normalized) noiseless data');
...
for ind = 1:4
    ...
    norReconOb = norReconOb./sum(norReconOb(:))*sum(norOb(:));
    norReconOb(norReconOb < 0) = 0;
...
recVars = retVars;
recVars{end+1} = norOb;
IterCompareFig = XYXZSubplot( recVars, z2BF, y2BF, ...
                                ["50 Iter", "100 Iter", "150 Iter", "200 Iter", "True Object"],...
                                "Comparison of MBPC with regularization results at different number of iterations for noiseless data",...

...
figure;
subplot(1,2,1);
imagesc(squeeze(recVars{end-1}(:,:,257,1,1))) ; axis image; colormap gray; xlabel('x'); ylabel('y');
...
for thetaVal = theta
    ...
    if (offX+round(abs(r.*cos(thetaVal))) ~= curX || offY+round(abs(r.*sin(thetaVal))) ~= curY)
        GWF1Val = [GWF1Val, recVars{end-1}(offX+round(abs(r.*cos(thetaVal))), offY+round(abs(r.*sin(thetaVal))), 257)];
        GWF2Val = [GWF2Val, recVars{end  }(offX+round(abs(r.*cos(thetaVal))), offY+round(abs(r.*sin(thetaVal))), 257)];
...
subplot(1,2,2); plot(obVal,'LineWidth',3); axis square;
hold on; plot(GWF1Val,'LineWidth',3);
hold on; plot(GWF2Val,'LineWidth',3);

...
saveas(TrueVsGWF, "TunableTrueVsGWF.jpg");

[ MSE, SSIM ] = MSESSIM(norReconOb, norOb)

%% compare between true vs recon vs WF
figure;
         plot(squeeze(norOb     (y2BF, :, z2BF)), 'DisplayName', 'True');
hold on; plot(squeeze(norReconOb(y2BF, :, z2BF)), 'DisplayName', 'GWF');
hold on; plot(squeeze(WF        (y2BF, :, z2BF)), 'DisplayName', 'WF');
xlabel('x'); ylabel('Intensity'); caxis(colorScale); legend;

figure;
         plot(squeeze(norOb     (y2BF, y2BF-12, :)), 'DisplayName', 'True');
hold on; plot(squeeze(norReconOb(y2BF, y2BF-12, :)), 'DisplayName', 'GWF');
hold on; plot(squeeze(WF        (y2BF, y2BF-12, :)), 'DisplayName', 'WF');
xlabel('z'); ylabel('Intensity'); legend;
...
expNames = ["201908280842_Sim3WGWFMultiOb256SNR15dB",...
...
saveas(MethodCompareFig, "3W15dBBestMethodComparison.jpg");
...
subplot(2,1,1);
plot(squeeze(norOb(y2BF, :, z2BF)), 'DisplayName', 'Truth', 'LineWidth', 2);
for ind = 2:4
    norReconOb = recVars{ind};
    hold on; plot(squeeze(norReconOb(y2BF, :, z2BF)), 'DisplayName', titTxt(ind-1), 'LineWidth', 2);
end
...
%ProfileCompareFig = figure('Position', get(0, 'Screensize'));
subplot(2,1,2);
plot(squeeze(norOb(y2BF, y2BF-14, :)), 'DisplayName', 'Truth', 'LineWidth', 2);
for ind = 2:4
    norReconOb = recVars{ind};
    hold on; plot(squeeze(norReconOb(y2BF, y2BF-14, :)), 'DisplayName', titTxt(ind-1), 'LineWidth', 2);
end
...
load(CIRLDataPath + "\Results\ISBI2020\" + expNames(1) + "\" + expNames(1) + ".mat", 'g');

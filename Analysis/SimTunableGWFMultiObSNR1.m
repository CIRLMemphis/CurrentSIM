...
expName = '201908211718_SimTunableGWFMultiOb256SNR1';
...
xlabel('x'); ylabel('z'); title('Zoomed-in middle'); caxis(colorScale);
...
% saveas(ForwardFig, "TunableSNR1ForwardImage.jpg");
...
hold on; plot(squeeze(norReconOb(y2BF, :, z2BF)), 'DisplayName', 'GWF'); caxis([0 1]);
xlabel('x'); ylabel('Intensity'); caxis(colorScale); legend;
...
hold on; plot(squeeze(norReconOb(y2BF, y2BF-12, :)), 'DisplayName', 'GWF');
...
suptitle('Comparison of the true object and GWF SNR1 reconstructed image');

saveas(TrueVsGWF, "TunableSNR1TrueVsGWF.jpg");

...
expName = 'SimTunableMBMultiOb256Iter200';
...
vars = [{norOb}, retVars{2:2:end}];
for k = 1:length(vars)
    vars{k} = vars{k}./sum(vars{k}(:))*sum(HROb(:));
    vars{k}(vars{k} < 0) = 0;
end
TruevsIterFig = XYXZSubplot(vars, z2BF, y2BF, ...
                            ["True Object", "40 Iter", "80 Iter", "120 Iter", "160 Iter", "200 Iter"],...
                            "MB different iterations", colormapSet, midSlice, colorScale);
saveas(TruevsIterFig, "TunableMBTruevsIterIter200.jpg");


%% compare original ob vs GWF results
norReconOb = vars{k}./sum(vars{k}(:))*sum(HROb(:));
...
saveas(TrueVsGWF, "TunableTrueVsMBIter200.jpg");

...
dXY     = 0.040/factr;     % lateral voxel scaling in microns ---- for 20X lens (6.4/20)....for 60X lens (6.4/63)
dZ      = 0.040/factr;     % axial voxel scaling in microns
uc      = 2*psfpar.NA/psfpari.v(1);  % cycle/um
offs    = [0, 0, 0];
phi     = [0, 1, 2]*(2*180)/3;
theta   = [0, 60, 120];
omegaXY = [0.6, 0.6, 0.6]*uc;
x0      = 0.122;% in mm
fL1     = 100;  % in mm
fL2     = 250;  % in mm
fMO     = 160/63;
...
xlabel('z'); ylabel('value'); suptitle("Visibility and PSF at zBF = " + num2str(zBF)); legend; axis tight; grid on;
jm2 = squeeze(jm(1+Y/2,:,1,1,1,2));
figure;  plot(jm2, 'DisplayName', 'jm(2)'); 
xlabel('x'); ylabel('value'); suptitle("jm at zBF = " + num2str(zBF)); legend;axis tight; grid on;